...
class purchase_requisition(osv.osv):
    ...
    def open_product_line(self, cr, uid, ids, context=None):
        ...
        action = self.pool.get('ir.actions.act_window').for_xml_id(cr, uid ,'purchase_requisition','purchase_line_tree', context=context)
        po_lines = self.browse(cr, uid, ids, context=context)[0].po_line_ids
        action['context'] = {
            'search_default_groupby_product' : True,
            'search_default_hide_cancelled': True,
        }
        action['domain'] = [('id','in', [line.id for line in po_lines])]
        return action
    ...
    def generate_po(self, cr, uid, id, context=None):
        """
        Generate all purchase order based on selected lines, should only
        be called on one tender at a time
        """
        if isinstance(id, (list, tuple)):
            assert len(id) == 1, "Only 1 ID expected, got: %r" % id
            id = id[0]
        po_obj = self.pool.get('purchase.order')
        po_line_obj = self.pool.get('purchase.order.line')
        id_per_supplier = {}
        tender = self.browse(cr, uid, id, context=context)
        if tender.state == 'done':
            raise orm.except_orm(
                _('Warning'),
                _('You have already generated the purchase order(s).'))

        # check that we have at least confirmed one line
        if tender.po_line_ids:
            if not any(po_line.state == 'confirmed' for po_line
                       in tender.po_line_ids):
                raise osv.except_osv(
                    _('Warning'),
                    _('You have no line selected for buying.'))
        ...
        accepted_states = ('draft', 'sent', 'bid', 'cancel')
        for po_line in tender.po_line_ids:
            #only take into account confirmed line that does not belong
            #to already confirmed purchase order
            if (po_line.state == 'confirmed' and
                    po_line.order_id.state in accepted_states):
                partner_id = po_line.partner_id.id
                id_per_supplier.setdefault(partner_id, []).append(po_line)

        #generate po based on supplier and cancel all previous RFQ
        for supplier, product_line in id_per_supplier.iteritems():
            # copy a quotation for this supplier and change order_line
            # then validate it
            quotation_id = po_obj.search(cr, uid,
                                         [('requisition_id', '=', tender.id),
                                          ('state', '=', 'bid'),
                                          ('partner_id', '=', supplier)],
                                         limit=1)
            if not quotation_id:
                raise orm.except_orm(_('No bid Encoded'),
                                     _('Workflow is broken please contact an administrator'))
            quotation_id = quotation_id[0]
            vals = self._prepare_po_from_tender(cr, uid, tender, context=context)
            new_po_id = po_obj.copy(cr, uid, quotation_id,
                                    default=vals, context=context)
            # duplicate po_line and change product_qty if needed and
            # associate them to newly created PO
            for line in product_line:
                vals = self._prepare_po_line_from_tender(cr, uid, tender,
                                                         line, new_po_id,
                                                         context=context)
                po_line_obj.copy(cr, uid, line.id,
                                 default=vals,
                                 context=context)
                #set previous confirmed line to draft
                po_line_obj.action_draft(cr, uid, line.id, context=context)
            #use workflow to set new PO state to confirm
            self.trigger_validate_po(cr, uid, new_po_id, context=context)
...
class purchase_order(osv.osv):
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        if not default:
            default = {}
        default.setdefault('requisition_id', False)
...
class purchase_order_line(osv.osv):
    ...
    _columns= {
        'quantity_bid': fields.float('Selected Quantity', digits_compute=dp.get_precision('Product Unit of Measure')),
    }

    _sql_constraints = [
        ('quantity_bid', 'CHECK(quantity_bid<=product_qty)', 'Selected quantity must be less or equal than the quantity in the bid!'),
    ]
